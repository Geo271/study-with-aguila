...
export function useLounge({ loungeCode, user }) {
  ...
  const [isMoving,     setIsMoving]     = useState(false) // ← exported for walking animation
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Export isMoving for walking animation
      setIsMoving(moved)
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'position' }, ({ payload }) => {
      ...
      // Also receive isMoving from other users for their walking animation
      setOtherUsers(prev => ({
        ...prev,
        [payload.userId]: {
          ...(prev[payload.userId] || {}),
  ...
  return {
    ...
    isMoving,   // ← walking animation for local player
  }
}
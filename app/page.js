...
import { askDocument, archiveSession, getSessions, getChatHistory } from './actions/chat'
...
export default function Home() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const result = await getSessions(session.user.id)
  ...
  const handleStartSession = async () => {
    ...
    setMessages([{
      role: 'ai',
      content: `Session **"${autoTitle}"** is ready! I've processed ${docIds.length} PDF${docIds.length > 1 ? 's' : ''} and I'm ready to help you study. Ask me anything or generate a quiz! 🎯`
    }])

    const updated = await getSessions(user.id)
    if (updated.success) setSessions(updated.sessions)
  }
  ...
  const handleGenerateQuiz = async () => {
    ...
    const result = await generateQuiz(uploadedDocs[0], user.id)
  ...
  const loadSession = async (session) => {
    ...
    setMessages([{
      role: 'ai',
      content: `Loading your conversation history for **"${session.title}"**... 🦅`
    }])
            ...
            <div className="bg-neutral-900 p-8 rounded-2xl border border-neutral-800 w-full max-w-md">
              <div className="text-center mb-6">
                ...
                <p className="text-neutral-400 text-sm mt-1">
                  Upload up to 3 PDF notes — session is named automatically
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl disabled:opacity-50 hover:bg-indigo-700 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                  {status === 'uploading' ? (
                    <>
                  ...
                  <div className="space-y-2 max-h-44 overflow-y-auto">
                    {sessions.map(s => (
                      <button key={s.id} onClick={() => loadSession(s)}
                        className="w-full text-left p-3 bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 hover:border-indigo-500/40 rounded-xl transition-all group flex items-center gap-3">
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-neutral-900/50 rounded-2xl border border-neutral-800 min-h-[400px]">
            <div className="px-4 py-2.5 border-b border-neutral-800 flex items-center justify-between">
              ...
              <button onClick={handleNewSession}
                className="text-xs text-neutral-500 hover:text-indigo-400 transition-colors flex items-center gap-1 flex-shrink-0">
                + New session
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {messages.map((msg, i) => (
                <div key={i} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'ai' && (
                    <div className="w-8 h-8 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center flex-shrink-0 text-base">
                      🦅
                    </div>
                  )}
          ...
          <div className="relative ml-auto w-full max-w-sm bg-neutral-900 border-l border-neutral-800 h-full flex flex-col">
            ...
            <div className="flex justify-between items-center p-5 border-b border-neutral-800">
              <div>
                ...
                <p className="text-xs text-neutral-500 mt-0.5">Tap a session to continue · Archive to hide it</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {sessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-neutral-500 text-sm text-center gap-3 py-12">
                  ...
                  <p>No sessions yet.<br/>Upload a PDF to get started.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {sessions.map(s => (
                    <div key={s.id}
                      className="group bg-neutral-800 border border-neutral-700 hover:border-indigo-500/40 rounded-xl transition-all overflow-hidden">
                      <button onClick={() => loadSession(s)}
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-neutral-500 mt-0.5">
                            {s.session_documents?.length || 0} PDF{s.session_documents?.length !== 1 ? 's' : ''}
                          </p>
                        </div>
                        <span className="text-xs text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity mt-1 flex-shrink-0">
                          Open
                        </span>
                      </button>

                      <div className="px-4 pb-3 flex justify-end opacity-0 group-hover:opacity-100 transition-opacity -mt-1">
                        <button
